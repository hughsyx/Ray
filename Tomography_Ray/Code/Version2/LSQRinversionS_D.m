...
function [m_c,lambda_c,InvTime1,RMS]=LSQRinversionS_D(plotLcurve,G,d,k0,k1,xi,zi,Nsrc,Nrec,frzx,lambda_s)
...
nxi=length(xi);
nzi=length(zi);
Dx1=diag([ones(nxi-1,1);0])-diag(ones(nxi-1,1),1);Dx1=Dx1(1:nxi-1,:);
Dx=kron(Dx1,eye(nzi));
Dz1=diag([ones(nzi-1,1);0])-diag(ones(nzi-1,1),1);Dz1=Dz1(1:nzi-1,:);
Dz=kron(eye(nxi),Dz1);
