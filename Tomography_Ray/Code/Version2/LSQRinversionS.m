...
function [m_c,lambda_c,InvTime1,RMS]=LSQRinversionS(G,d,k0,k1,xi,zi,Nsrc,Nrec,lambda_s)
if (nargin==6)
    Nsrc=1;Nrec=length(d);lambda_s=-10000;
end
if (nargin==8)
    lambda_s=-10000;
end
...
%sm_tol=s(1)/10000; % set smallest singular value tolerance
...
if lambda_s>0
    lambda_lc=lambda_s;
    tic
    [xl]=lsqr_b([G;lambda_lc*eye(size(G,2))],[d-G*m0;zeros(size(G,2),1)],k1);
    ...
    [ dml,lambda_lc,rho_l,eta_l,lambda_l,rhoc_iter,InvTime1 ] = SelectModel_Lcurve( G, U, sm, d-G*m0, k1 );
