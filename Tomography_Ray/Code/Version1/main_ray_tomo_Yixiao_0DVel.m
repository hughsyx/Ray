...
load([Tomo_path, '/Models/cross_well_att_init.mat']);
load([Tomo_path, '/Models/cross_well_att_T_obs_new.mat']);
Vtrue = cross_well_att_vp;
Vsm = cross_well_att_init;
T_obs = t_esti_model2';
clear('cross_well_att_vp','cross_well_att_init','t_pred');
...
nz_G=2; nx_G=2; % parameter for inversion
deltax_G = 139; deltaz_G=1079;
...
% cvx_end
% toc;
%% save 0D model as the initial for 1D inversion
V0D = V_collec(:,:,end);
save([Tomo_path, '/Models/cross_well_att_0Dvp.mat'],'V0D')
%% plot
figure;
pcolor(Vtrue);
shading interp;
